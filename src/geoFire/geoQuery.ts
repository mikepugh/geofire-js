   ...
   */
	private _childAddedCallback(locationDataSnapshot: firebase.database.DataSnapshot): void {
		this._updateLocation(
			geoFireGetKey(locationDataSnapshot),
			decodeGeoFireObject(locationDataSnapshot.val()),
			decodeGeoFireDataObject(locationDataSnapshot.val())
		);
	}
   ...
   */
	private _listenForNewGeohashes(): void {
		...
		geohashesToQuery.forEach((toQueryStr: string) => {
			...
			const firebaseQuery: firebase.database.Query = this._firebaseRef
				.orderByChild('g')
				.startAt(query[0])
				.endAt(query[1]);
